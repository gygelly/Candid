Can = {
  ...
  can: function (action, subject, user) {
  ...
  _run: function (action, subject, user, conditionOverrides) {
    ...
    var passes = false
    if (action === 'db') {
      passes = Can._run('insert', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('read', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('update', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('remove', subject, user, conditionOverrides)
    } else if (action === 'html') {
      passes = Can._run('get', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('post', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('put', subject, user, conditionOverrides)
      if (passes) { return passes }
      passes = Can._run('delete', subject, user, conditionOverrides)
    } else {
